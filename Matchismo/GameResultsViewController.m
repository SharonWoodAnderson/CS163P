...
- (void)updateUI
{
    ...
    for (GameResults *result in [[GameResults allGameResults] sortedArrayUsingSelector:self.sortSelector]) { // sorted
        // displayText = [displayText stringByAppendingFormat:@"Score: %d (%@, %0g)\n", result.score, result.end, round(result.duration)]; // version in lecture
        displayText = [displayText stringByAppendingFormat:@"Score %@: %d (%@, %0g)\n", result.gameType ? result.gameType : @"Card Matching", result.score, [formatter stringFromDate:result.end], round(result.duration)];  // formatted date
    }
